...
define([
  ...
  'zui',
  'warn'
], function ($, validator, utils, http, lazyload) {
  const params = new URLSearchParams(window.location.search)
  lazyload.load_image()
  function show_modal_type(index) {
    $('#login_modal')
      .find('.dx-tabs .dx-tab-content')
      .eq(index)
  ...
  function search(keyword, page_type) {
    ...
    switch (page_type) {
      case 'mv':
        if (type === 'author') {
          url = `all?merchant=${keyword}`
        } else {
          url = `mv/all?${type}=${keyword}`
        }
        break
      case 'video_list':
        url = `video_list.html?${type}=${keyword}`
        break
      case 'novel_list':
        url = `novel_list.html?${type}=${keyword}`
        break
      case 'comic_list':
        url = `comic_list.html?${type}=${keyword}`
        break
      default:
        url = `all?${type}=${keyword}`
    }
  ...
  $('#report_modal .btn-submit-report').on('click', async function () {
    var type = $('input[name="report_item"]:checked')
      .map(function () {
        return this.value
      })
      .get()
...
function calc_scroll_height_percent() {
  ...
  if (scrollTop > lastScrollTop) {
    ...
    $('.dx-backtop .dx-backtop--arrow').css({
      transform: 'translate(-50%, -50%) rotate(180deg)', // 直接设置样式
      transition: 'transform 0.3s' // 添加过渡效果
    })
  } else if (scrollTop < lastScrollTop) {
    console.log('向上滚动')
    $('.dx-backtop .dx-backtop--arrow').css({
      transform: 'translate(-50%, -50%) rotate(0deg)', // 恢复样式
      transition: 'transform 0.3s' // 添加过渡效果
    })
  }
...
$(window).on('scroll', utils.throttle(calc_scroll_height_percent, 50))
  ...
  $('#tip_modal').on('hidden.zui.modal', function () {
    // 设置 cookie
    document.cookie = `${landing_modal_key}=${now}; path=/;`
  ...
  const copyText = (function () {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      ...
      return function (text, autoToast = true) {
