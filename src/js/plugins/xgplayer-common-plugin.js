define(['xgplayer', 'lib/hls'], function() {
  const { Events, BasePlugin, Plugin } = window.Player
  ...
  class HlsJsPlugin extends BasePlugin {
    ...
    static get defaultConfig() {
      return {
        hlsOpts: {
          // enableWorker: true,
          // workerPath: '/js/lib/hls-worker.js',
        }
      }
    }
        ...
        if (data.fatal) {
          switch (data.type) {
            case Hls.ErrorTypes.NETWORK_ERROR:
              if (!(data?.networkDetails?.status === 404 || data?.networkDetails?.status === 403)) {
                this.hls.startLoad()
              }
              break
