...
requirejs(
  ['jquery', 'plugins/lazyload', 'validate', 'utils', 'http', 'lib/cropper', 'common'],
  function ($, lazyload, validator, utils, http, Cropper) {
    var cropper = null
    ...
    const API_MAP = {
      name: { url: '/index/resetMemberInfo' }
    }
    const MESSAGE_TYPE = {
      name: '昵称不能为空'
    }
    $('.btn-open').on('click', function () {
      ...
      $modal.find(`.${type}`).show().siblings('div').hide()
      if (type === 'gender') {
