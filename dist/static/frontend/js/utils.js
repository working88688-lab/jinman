...
define(['http', 'lib/clipboard', 'jquery', 'zui', 'lib/qrcode'], function (http, Clipboard) {
  ...
  function formatNumberWithUnit() {
    ...
    // 返回带单位的格式化数字
    return "".concat(formattedNumber).concat(units[unitIndex]);
  }
  function format_url_search_params() {
    let merge_params = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    const params = new URLSearchParams(window.location.search);
    const result = {};
    for (const [key, value] of params.entries()) {
      result[key] = value;
    }
    const merged_params = _objectSpread(_objectSpread({}, result), merge_params);
    const entries = Object.entries(merged_params);
    const paramString = entries.map(_ref => {
      let [key, value] = _ref;
  ...
  function check_login(callback) {
    let show_modal = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  ...
  function useShare(url) {
    let selector = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '.btn-share';
    let tips = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '分享链接复制成功';
  ...
  function useQrcode(text) {
    let el = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '#qrCode';
    let config = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    new QRCode(document.querySelector(el), _objectSpread({
      text,
      width: 120,
      height: 120,
      colorDark: '#000000',
  ...
  function useMessage(msg) {
    ...
    let config = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    new $.zui.Messager(msg, _objectSpread({
      time: 3000,
      type,
      close: false
    }, config)).show();
