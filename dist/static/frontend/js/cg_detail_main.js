...
requirejs(['player', 'plugins/lazyload', 'http', 'utils', 'lib/clipboard', 'lib/handlebars', 'jquery', 'video_helper', 'common'], function (player, lazyload, http, utils, Clipboard, Handlebars) {
  let url = '';
  try {
    if (typeof track_vtt) {
      const blob = new Blob([track_vtt], {
        type: 'text/plain'
      });
  ...
  $('.dx-tabs').on('click', '.dx-tab', function () {
    ...
    $this.addClass('dx-tab--active').siblings().removeClass('dx-tab--active').parents('.dx-tab-list').siblings('.dx-tab-content').removeClass('dx-tab-content--active').eq(index).addClass('dx-tab-content--active');
  ...
  $('.video-like').on('click', async function () {
    ...
    try {
      ...
      let html = '<svg class="w-5 h-5 ' + svg_class + ' svg-icon mr-1">' + '<use href="/static/frontend/icons/icons.svg#like"></use></svg>' + '<span>' + result['like_count'] + '</span>';
  ...
  $('.collect-video').on('click', async function () {
    ...
    try {
      ...
      const html = '<svg class="w-5 h-5 svg-icon mr-1">\n' + '                                            <use href="/static/frontend/icons/icons.svg#collect"></use>\n' + '                                        </svg>' + collect_txt;
      $(this).html(html);
      const collect_div = '<svg class="w-5 h-5 svg-icon mr-1\n' + text_primary + '">\n' + '<use href="/static/frontend/icons/icons.svg#heart"></use>\n' + '</svg>' + result['collect_count'];
