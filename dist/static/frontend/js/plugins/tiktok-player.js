...
define(['plugins/lazyload', 'utils', 'plugins/xgplayer-common-plugin', 'xgplayer'], function (lazyload, utils, common_plugins) {
  ...
  function create_player() {
    ...
    if (document.createElement('video').canPlayType('application/vnd.apple.mpegurl')) {
      // 原生支持 hls 播放
      player = new Player(_objectSpread(_objectSpread({}, base_config), restConfig));
    } else if (HlsJsPlugin.isSupported()) {
      // 第一步
      base_config.plugins.push(HlsJsPlugin), player = new Player(_objectSpread(_objectSpread({}, base_config), restConfig));
    }
    if (poster) {
      lazyload.decrypto_image(poster).then(_url => {
        player.poster = _url;
