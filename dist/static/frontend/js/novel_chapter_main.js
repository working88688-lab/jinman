...
requirejs(['plugins/lazyload', 'http', 'utils', 'player', 'jquery', 'common'], function (lazyload, http, utils, player) {
  ...
  try {
    replaceStrArray = JSON.parse(localStorage.getItem(REPLACE_KEY)) || [{
      origin: '',
      target: ''
    }];
  } catch (error) {
  ...
  function get_user_mark() {
    http({
      ...
      if (chapter_no && items.some(function (item) {
        return item.chapter_no === Number(chapter_no);
      })) {
        $('.btn-mark').addClass('text-primary');
      }
      if (items.length) {
        $('.directory .marks .lists').html(items.map(function (item) {
          return $("<li class=\"volume_box\"><a href='novel_chapter.html?novel_id=".concat(item.novel_id, "&chapter_no=").concat(item.chapter_no, "' class=\"px-3 py-2 flex items-center justify-between cursor-pointer\"><span>").concat(item.node_title, "</span>\n              <div class=\"flex-center\"><span class=\"mr-1\">").concat(item.created_at, "</span>\n                <svg class=\"w-4 h-4\">\n                  <use class=\"open\" href=\"/static/frontend/icons/icons.svg#trash\"> </use>\n                </svg>\n              </div></a></li>"));
        }));
      }
    });
  }
  function get_audio_list() {
    http({
      ...
      if (items.length) {
        $('.dx-popup .music .lists').html(items.map(function (item, index) {
          return $("<button data-src='".concat(item, "' class=\"flex play-music items-center px-3 py-2 dx-hairline--bottom\">\n              <svg class=\"w-4 h-4 mr-1\">\n                <use class=\"open\" href=\"/static/frontend/icons/icons.svg#play_music\"></use>\n              </svg><span>\u80CC\u666F\u53EB\u5E8A\u58F0").concat(index + 1, "</span>\n            </button>"));
        }));
      }
    });
  }
  ...
  function replaceTargetContent() {
    $.each($('.line'), function () {
      var text = $(this).text();
      replaceStrArray.filter(item => item.origin.length && item.target.length).forEach(item => {
  ...
  function renderReplace() {
    var html_string = replaceStrArray.map((item, index) => {
      return "<div class=\"flex items-center whitespace-nowrap px-2 mb-3\">\u5C06\n                <input class=\"w-[100px] mx-1 border border-solid border-border\" type=\"text\" data-type=\"origin\" value=\"".concat(item.origin, "\">\u66FF\u6362\u6210\n                <input class=\"w-[100px] ml-1 border border-solid border-border\" type=\"text\" data-type=\"target\" value=\"").concat(item.target, "\">\n                <svg class=\"w-6 h-6 delete\" data-index='").concat(index, "'>\n                  <use class=\"open\" href=\"/static/frontend/icons/icons.svg#remove\"></use>\n                </svg>\n              </div>");
    }).join('');
  ...
  function set_color_theme(color, index) {
    ...
    $('.custom-theme').eq(index - 1).addClass('is-select').siblings().removeClass('is-select');
  }
  ...
  $('.btn-theme-submit').on('click', function () {
    if (localStorage.getItem('is_login')) {
      ...
      localStorage.setItem('novel_setting', JSON.stringify({
        font_size: defaultFontSize,
        theme: $('.custom-theme.is-select').index()
      }));
      utils.useMessage('保存成功', 'success');
  ...
  $('.btn-theme-reset').on('click', function () {
    ...
    const color = $('.custom-theme').eq(setting.theme - 1).data('color');
